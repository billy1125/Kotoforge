...
3. **維護「§0 素材檔案庫索引與搜尋協定」**：`personal-voice.md` 採**索引＋蒸餾兩層**設計。§0 是給 AI 的檢索地圖，須隨 sources 變動更新——一份**檔案庫清單表**（每個來源檔：檔名／內容與分類／何時該搜尋它）＋每檔**分類錨點**（`grep -n "^## \|^### " <檔>` 取得的標題作穩定錨點，行號僅輔助、會位移）＋一段**搜尋協定**（grep/Read 範例指令）。目的：蒸餾必有損失，索引讓完整素材始終可檢索，尤其素材增多時。
4. **萃取並寫入 `writing-style/personal-voice/personal-voice.md`**：用下方模板；每條盡量附一個從素材觀察到的短例（節錄 <15 字，不整段照抄）。**更新而非重寫**——保留使用者手動補充區。
...
> 本檔為「索引＋蒸餾」兩層：§0 檢索地圖，§1 起為蒸餾筆法；規則不夠時 AI 隨時 grep sources/ 原文。
