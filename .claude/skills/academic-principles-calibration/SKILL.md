...
4. **維護「§0 素材檔案庫索引與搜尋協定」**：`academic-principles.md` 採**索引＋蒸餾兩層**設計。§0 是給 AI 的檢索地圖，須隨 sources 變動更新：
   - 一份**檔案庫清單表**（每個來源檔：檔名／內容與分類／何時該搜尋它）。
   - 每檔的**分類錨點**：以 `grep -n "^## " <檔>` 取得的分類標題（中英）作穩定錨點；行號可附但註明「僅輔助、會位移」。
   - 一段**搜尋協定**（grep/sed/Read 範例指令），讓後續編修能隨時回原文取更完整的詞彙、句型與解說，而非只靠蒸餾。
   目的：蒸餾必有損失（如 200 詞、150 句型無法全入摘要），索引讓完整素材**始終可檢索**。
5. **寫入 `writing-style/academic-principles/academic-principles.md`**：用下方模板，條列可查閱；每條盡量附一個短例。**更新而非重寫**——保留使用者手動補充區，標出本次新增／變動處。
6. **不刪除 sources**：原始文件一律保留。
7. **回報**：用繁體中文簡述抓到的 3–5 個關鍵原則，提醒使用者可自行補充或覆寫任一條，並說明 §0 索引已更新、AI 可隨時搜尋原始素材。
...
> 本檔為「索引＋蒸餾」兩層：§0 檢索地圖，§1 起為蒸餾規則；規則不夠時 AI 隨時 grep sources/ 原文。

## 0. 素材檔案庫索引與隨時搜尋協定（AI 用）
- 檔案庫清單表（檔名｜內容與分類｜何時搜尋）
- 每檔分類錨點（grep "^## " 取得的標題；行號僅輔助、會位移）
- 搜尋協定（grep/sed/Read 範例指令）
## 1. 語域與情態語氣強度
## 2. 動詞語氣階梯（依證據強度）
...
## 4. 段落與過渡（主題句、連接詞分類）
## 5. 詞彙與搭配（含歸因 vs 背書強度）
## 6. 章節功能句型庫（依論文段落功能，move-based；若有句型素材）
## 7. 常見禁用／改寫
