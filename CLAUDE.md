...
3. **產生建議**：
   - 單段 → 用 `passage-revision`，給 ≤5 版本。**基本原則：一律多版本（≥2）＋附一句「我的建議」，不得因連續／批次／逐段而退成單一版；見該 skill 產出規則。**
   - 一個範圍的整體彙整 → 用 `section-review`，給方向選項（此 skill 給「方向」不給逐段版本；要版本回到 `passage-revision`）。
